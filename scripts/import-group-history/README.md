...
**Один запуск одновременно.** Сессия Telethon — это SQLite; два параллельных процесса (`терминал + Cursor`, два окна и т.п.) дают `database is locked`, «wrong session ID» и обрывы скачивания. Скрипт берёт файл **`gents_import_session.lock`** (не коммитится): второй запуск сразу завершится с сообщением. Не запускайте импорт и в IDE, и вручную одновременно.
